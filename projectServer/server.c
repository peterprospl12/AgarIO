...
struct Cell revive_player() // nic tu sie chyba zlego nie stanie
{
    struct Cell cell;
...
void *connection_handler(void *socket_desc)
{
	...
	do {
		clientCell = recieve_cell(sock); // jest obsulga bledow juz w funkcji ( jak blad to konczy socket i watek )
		if(clientCell.id == -1)
		{
